"""
ou yeah loss functions :D
"""
import tensorflow as tf

def tf_mse(predic,label,name='mse_loss',is_metric_mse=True):
    if is_metric_mse:
        return tf.metrics.mean_squared_error(predictions=predic,labels=label, name=name)
    else:
        with tf.name_scope(name)as scope:
            return tf.losses.mean_squared_error(labels=label, predictions=predic)
